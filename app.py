...
def draw_graph(n,m):
    ...
    random_nodes= list(random.choice(list(G.nodes),size=G.number_of_nodes(),replace=False))          
    ...
    for i in range(G.number_of_nodes()):
        for j in range(G.number_of_nodes()):
            ...
            df=pd.DataFrame(matrix,index=range(1,G.number_of_nodes()+1),columns=range(1,G.number_of_nodes()+1))
...
edges=input(f"Introduzca el número de aristas como un número entero entre {nodes} y {nodes*(nodes-1)}:>>> ")
while not edges.isnumeric() or not int(edges) in range(nodes,(nodes*(nodes-1))+1):
    edges=input(f"Introduzca el número de aristas como un número entero entre {nodes} and {nodes*(nodes-1)}:>>> ")
